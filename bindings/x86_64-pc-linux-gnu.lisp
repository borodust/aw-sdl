...
(defparameter %sdl::+audio-driver-alsa-dynamic+ "libasound.so.2")
...
(defparameter %sdl::+audio-driver-jack-dynamic+ "libjack.so.0")

(defparameter %sdl::+audio-driver-oss+ 1)

(defparameter %sdl::+audio-driver-pulseaudio+ 1)

(defparameter %sdl::+audio-driver-pulseaudio-dynamic+ "libpulse-simple.so.0")
...
(defparameter %sdl::+file+ "/tmp/tmpKPIJSXFR-tmp.h")
...
(defparameter %sdl::+libsamplerate-dynamic+ "libsamplerate.so.0")

(defparameter %sdl::+lil-endian+ 1234)

(defparameter %sdl::+line+ 364)
...
(defparameter %sdl::+prix64+ "lX")

(defparameter %sdl::+pr-is64+ "ld")

(defparameter %sdl::+pr-iu64+ "lu")

(defparameter %sdl::+pr-ix64+ "lx")
...
(defparameter %sdl::+video-driver-kmsdrm-dynamic+ "libdrm.so.2")

(defparameter %sdl::+video-driver-kmsdrm-dynamic-gbm+ "libgbm.so.1")

(defparameter %sdl::+video-driver-wayland+ 1)

(defparameter %sdl::+video-driver-wayland-dynamic+ "libwayland-client.so.0")

(defparameter %sdl::+video-driver-wayland-dynamic-cursor+ "libwayland-cursor.so.0")

(defparameter %sdl::+video-driver-wayland-dynamic-egl+ "libwayland-egl.so.1")

(defparameter %sdl::+video-driver-wayland-dynamic-xkbcommon+ "libxkbcommon.so.0")
...
(defparameter %sdl::+video-driver-x11-dynamic+ "libX11.so.6")

(defparameter %sdl::+video-driver-x11-dynamic-xcursor+ "libXcursor.so.1")

(defparameter %sdl::+video-driver-x11-dynamic-xext+ "libXext.so.6")

(defparameter %sdl::+video-driver-x11-dynamic-xinerama+ "libXinerama.so.1")

(defparameter %sdl::+video-driver-x11-dynamic-xinput2+ "libXi.so.6")

(defparameter %sdl::+video-driver-x11-dynamic-xrandr+ "libXrandr.so.2")

(defparameter %sdl::+video-driver-x11-dynamic-xss+ "libXss.so.1")

(defparameter %sdl::+video-driver-x11-dynamic-xvidmode+ "libXxf86vm.so.1")
...
(cffi:defcenum (%sdl::|C:@EA@SDL-DUMMY-ENUM| :unsigned-int)
               "/home/borodust/devel/repo/aw-projects/aw-sdl/src/lib/sdl/include/SDL_stdinc.h:343:9"
               (:dummy-enum-value 0))
...
(cffi:defcstruct (%sdl::|C:@S@SDL-SYS-W-MINFO@U@SDL-SYSWM.H@5693@S@SDL-SYSWM.H@6194|
                  :size 16)
                 (%sdl::display
                  (claw-utils:claw-pointer %sdl::display) :offset 0)
...
(cffi:defcstruct (%sdl::|C:@S@SDL-SYS-W-MINFO@U@SDL-SYSWM.H@5693@S@SDL-SYSWM.H@7760|
                 ...
                 (%sdl::display
                  (claw-utils:claw-pointer
                   (:struct %sdl::wl-display))
                  :offset 0)
                 (%sdl::surface
                  (claw-utils:claw-pointer
                   (:struct %sdl::wl-surface))
                  :offset 8)
                 (%sdl::shell-surface
                  (claw-utils:claw-pointer
                   (:struct %sdl::wl-shell-surface))
                  :offset 16))
...
(eval-when (:load-toplevel :compile-toplevel :execute)
  ...
  (export '%sdl::+video-driver-x11-const-param-xextadddisplay+ :%sdl)
